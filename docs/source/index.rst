Welcome to multiomicsintegrator: mcia usage
================================
...
correlation analysis `correlation <../modules/local/correlation>`__ to
estimate correlation between differentially expressed features. We
suggest to use the count matrices of the differentially expressed
features.
...
`comparative_analysis <../modules/local/comparative_analysis>`__ tool,
which estimates the semantic distance (e.g. the similarity of their
pathways) of two features signatures. Input is a txt file, with each
column storing one distinct feature signature. Available parameters are:
...
of CPUs, memory, and time for the whole pipeline. Therefor you can try
...
process, that were already calculated. If you can not increase the
...
which can be referenced in a configuration file to select and configure
